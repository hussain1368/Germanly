...
{
    public partial class PluralsStartPage : ContentPage
    {
        private readonly Color selectedBackground = Color.FromArgb("#F1ECFA");
        private readonly Color selectedStroke = Color.FromArgb("#6D4AA2");
        private readonly Color selectedText = Color.FromArgb("#6D4AA2");
        ...
        public PluralsStartPage()
        {
            ...
            selectedLevel = settingsService.PluralLevel;
            selectedMethod = settingsService.SelectedPluralTestMethod;
        ...
        private async void OnStartTestClicked(object sender, EventArgs e)
        {
            ...
            if (session.TotalCount == 0)
            {
                await ToastService.ShowAsync(
                    this,
                    $"No {GetSelectedLevelPrefix()}nouns with plural forms available.");
                return;
            }

            await Shell.Current.GoToAsync(nameof(PluralsTestPage));
        }
        ...
        private async void OnReviewMistakesClicked(object sender, EventArgs e)
        {
            ...
            if (session.TotalCount == 0)
            {
                await ToastService.ShowAsync(
                    this,
                    $"No {GetSelectedLevelPrefix()}plural mistakes to review yet.");
                return;
            }
            ...
            bool refreshStats = true)
        {
            ...
            if (persist)
            {
                settingsService.PluralLevel = selectedLevel;
            }
        ...
        private void SelectMethod(IrregularTestMethod method, bool persist = true)
        {
            ...
            if (persist)
            {
                settingsService.SelectedPluralTestMethod = method;
            }
