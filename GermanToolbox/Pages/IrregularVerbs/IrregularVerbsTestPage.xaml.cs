...
{
    public partial class IrregularVerbsTestPage : ContentPage
    {
        ...
        private readonly IrregularVerbDistractorGenerator distractorGenerator;
        ...
        public IrregularVerbsTestPage()
        {
            ...
            distractorGenerator = AppServices.GetRequiredService<IrregularVerbDistractorGenerator>();
        }
        ...
        protected override async void OnAppearing()
        {
            ...
            if (!ApplyCurrentWord())
            {
                await DisplayAlert("Kein aktiver Test", "Start an irregular verb test first.", "OK");
        ...
        private async void OnMultipleChoiceClicked(object? sender, EventArgs e)
        {
            ...
            if (session?.Mode != PracticeMode.IrregularVerb || currentWord is null)
            {
                return;
            }

            var isCorrect = IrregularVerbAnswerService.IsCorrect(
                currentWord,
        ...
        {
            var session = testSessionService.CurrentSession;
            if (session?.Mode != PracticeMode.IrregularVerb ||
        ...
        private async Task HandleAnswerAsync(bool isCorrect, bool completeOnIncorrect)
        {
            ...
            try
            {
                result = testSessionService.RecordIrregularAnswer(isCorrect, completeOnIncorrect);
            }
        ...
        {
            var session = testSessionService.CurrentSession;
            if (session?.Mode != PracticeMode.IrregularVerb || session.CurrentWord is null)
            {
                return false;
            }

            var word = session.CurrentWord.Word;
            VerbLabel.Text = word.Word;
            VerbLabel.FontSize = GenericHelper.GetPromptFontSize(word.Word);
            LevelLabel.Text = word.Level;
            var formTitle = GetFormTitle(session.IrregularVerbForm).ToUpperInvariant();
            FormLabel.Text = formTitle;
            RevealTitleLabel.Text = formTitle;
            ...
            BuildMultipleChoiceOptions(word, session.IrregularVerbForm);
            SetAnswerText(string.Empty);
            var displayAnswer = IrregularVerbAnswerService.GetDisplayAnswer(
                word,
                session.IrregularVerbForm);
        ...
        private void BuildMultipleChoiceOptions(WordEntry word, IrregularVerbForm form)
        {
            MultipleChoiceOptions.Children.Clear();
            var correctAnswer = IrregularVerbAnswerService.GetDisplayAnswer(word, form);
            var options = distractorGenerator
                .Generate(word, form, 3)
            ...
            foreach (var option in options)
            {
                var button = new Button
                {
                    ...
                    BorderColor = Color.FromArgb("#DCEBDD"),
                    BorderWidth = 2,
                    CornerRadius = 8,
        ...
        private async Task FinishSessionAsync()
        {
            ...
            await Shell.Current.GoToAsync(nameof(IrregularVerbsResultsPage));
            WordCard.Scale = 1;
            isAnimating = false;
        }
        ...
        {
            testSessionService.AbandonCurrentSession(PracticeMode.IrregularVerb);
            await HideQuitConfirmationAsync();
            await Shell.Current.GoToAsync("..");
        }
